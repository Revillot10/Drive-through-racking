...
/*In other cases, storage requests result from receiving a production batch and retrieval request 
 results from customer demand*/
    ...
    public static void main(String[] args) { 
      ...
      //CALCULATE ARRIVAL/DEPARTURE OF EACH PALLET                    
      for (int i=0; i<Trucks;i++){
            int randomDeparture,randomArrival,palletArrival,palletDeparture;             
            //ARRIVAL
            do{                 
                palletArrival=(r.nextInt(availableStages.size())); 
                randomArrival=(int) availableStages.get(palletArrival);
            } while(palletArrival>=availableStages.size()-1); 
                  for(int j=0;j<palletsTrucks;j++){
                      A.add(randomArrival);
                  }
                  availableStages.remove(palletArrival); 
                  System.out.println("Truck "+(i+1)+ " arrives at the stage "+randomArrival+" "+A );
                  System.out.println("Available Stages:"+availableStages);
            //DEPARTURE            
             do{
                 palletDeparture=(r.nextInt(availableStages.size()));
                 randomDeparture=(int) availableStages.get(palletDeparture);
             } while( (randomDeparture==1) || (randomDeparture<=randomArrival)); 
                  for(int j=0;j<palletsTrucks;j++){
                      D.add(randomDeparture); 
             }
             availableStages.remove(palletDeparture);  
             System.out.println("Truck "+(i+1)+ " leaves at the stage "+randomDeparture+" "+D);
             System.out.println("Available Stages:"+availableStages);
        } 
        System.out.println("Arrival RANDOM "+A);
        System.out.println("Departure RANDOM "+D); 
      ...
      PsiRBuilder.append("\n];");
        ...
        try{   
            ...
            BufferedWriter bufferedWriter= new BufferedWriter(new FileWriter ("Instancias\\DTPR_RANDOM.txt"));
    ...
    FIFOInstance Random = new FIFOInstance();
    }    
}
